...
async function rssImageShortcode(url, src) {
...
function dateShortcode(dateStr) {
  if (!dateStr) {
    return "";
  }
...
function consoleShortcode(arg) {
  console.log(">>>", arg);
  console.log("this.page", this.page);
  return `[${arg}]`;
}

function spriteShortcode(arg) {
  ...
  eleventyConfig.addCollection("weblogByMonth", (collection) => {
    ...
    return uniqueYears.reduce((prev, year) => {
      const filteredPosts = posts.filter(
        (post) => /(?<year>[0-9]+)-/.exec(post.filePathStem).groups.year === year
      );
