...
document.addEventListener("DOMContentLoaded", function(event) {
	var canvas = new Canvas($('#canvas-div'));
...
var Canvas = newClass(function() {
	...
	return {
		construct: function(container) {
			...
			this.pixelWidth = canvasElement.width = 550;
			this.pixelHeight = canvasElement.height = 363;
		...
		public: {
			...
			click: function(fn) {
				this.obj.addEventListener('click', function(evt) {
					var rect = canvas.obj.getBoundingClientRect();
...
var Controls = newClass(function() {
	var controls;
	...
	return {
		construct: function(canvas, shape, gameoflife) {
			...
			this.started = false;
			this.generation = 0;
			this.generationElement = $('#generation');
		},
		public: {
			init: function(shapes) {
				...
				shapesSelect.addEventListener('change', function(e) {
				...
				size.addEventListener('change', function() {
					...
					var newGridSize = Math.floor(size.options[size.selectedIndex].value);
				...
				speed.addEventListener('change', function() {
					controls.speed = speed.options[speed.selectedIndex].value;
					if (controls.started) {
						controls.animate();
					}
				});
...
var GameOfLife = newClass(function() {
	return {
		public: {
			next: function(shape) {
				var neighbours = {};
